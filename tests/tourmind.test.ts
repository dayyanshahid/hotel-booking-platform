...
describe("what counts as a facility", () => {
  /*
   * Their facility list is not only facilities. Payment methods, the property
   * type and distance readings arrive in the same array as the swimming pool,
   * and everything we do not recognise is kept verbatim — right for a real
   * amenity with no code of ours, wrong for these.
   *
   * It showed on a results card as "Meeting rooms · Distance from property
   * (ft) - 1640", and in the trade comparison every column read "hotel ·
   * American Express · MasterCard" — three identical rows of nothing, in a
   * panel built to tell three properties apart.
   */
  const facilitiesOf = (names: string[]) => {
    const record = recordFrom(staticList.HotelStaticListResult.Hotels[0])!;
    return normalizeTourmind(
      { slug: tourmindSlug(record.hotelId), record: { ...record, amenities: names }, offers: [] },
      intent,
      "en",
    ).hotel.amenities.map((a) => a.label);
  };

  it("drops the payment methods", () => {
    expect(facilitiesOf(["American Express", "MasterCard", "Visa", "EC"])).toEqual([]);
  });

  it("drops the property type, which is a field of its own", () => {
    expect(facilitiesOf(["hotel", "Apartment"])).toEqual([]);
  });

  it("drops a distance with nothing to measure from", () => {
    expect(facilitiesOf(["Distance from property (ft) - 1640"])).toEqual([]);
  });

  it("keeps a real facility it has no code for", () => {
    // The rule has to be a reject list, not an allow list: an amenity we do not
    // recognise is still one the guest is being offered.
    expect(facilitiesOf(["Rooftop cinema"])).toEqual(["Rooftop cinema"]);
  });

  it("still maps the ones we do recognise", () => {
    expect(facilitiesOf(["Outdoor Pool"])).toHaveLength(1);
  });
});
